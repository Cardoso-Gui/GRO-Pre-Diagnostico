...
async function clientList(reset=true){
 const ticket=++sequence;if(reset){offset=0;$('#client-options').replaceChildren();}
 $('#more-clients').hidden=true;say('Buscando clientes…');
 let query=authClient.from('clients').select('id,legal_name,trade_name,cnpj,cnae,address,contact_phone,contact_email').eq('archived',false).order('legal_name').order('id');
 const term=$('#client-filter').value.trim();
 if(term){const value=`"%${term.replace(/[\\%_]/g,'\\$&').replace(/\\/g,'\\\\').replace(/"/g,'\\"')}%"`;query=query.or(`legal_name.ilike.${value},trade_name.ilike.${value},cnpj.ilike.${value}`);}
 try{const {data,error}=await query.range(offset,offset+20);if(ticket!==sequence)return;if(error)throw error;
 for(const client of data.slice(0,20)){const button=document.createElement('button');button.type='button';button.className='client-choice';button.textContent=`${client.legal_name} · ${client.cnpj || 'Sem CNPJ'}`;button.addEventListener('click',()=>choose(client));$('#client-options').append(button);}
 offset+=Math.min(data.length,20);$('#more-clients').hidden=data.length<=20;say(offset?'Selecione uma empresa.':'Nenhum cliente encontrado. Cadastre o cliente antes de iniciar.');
 }catch{if(ticket===sequence)say('Não foi possível buscar clientes. Clique em Buscar para tentar novamente.',true);}
}
...
$('#create-assessment').addEventListener('submit',async event=>{
 ...
 saving=true;$('#start-assessment').disabled=true;say('Criando levantamento…');
 ...
 finally{saving=false;$('#start-assessment').disabled=false;}
});
async function loadForm(){
 ...
 for(const file of ['cnae-risk-map.js','esocial-risk-table.js','occupational-risk-table.js','training-rules.js','nr-report-rules.js','script.js'])await new Promise((resolve,reject)=>{const script=document.createElement('script');script.src=file;script.onload=resolve;script.onerror=reject;document.body.append(script);});
...
$('#client-search-form').addEventListener('submit',e=>{e.preventDefault();clientList();});$('#more-clients').addEventListener('click',()=>clientList(false));$('#more-drafts').addEventListener('click',()=>historyList(false));

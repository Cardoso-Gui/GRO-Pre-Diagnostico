...
async function load(reset=true){
 ...
 try{const {data,error}=await authClient.from('assessments').select('id,title,completed_at,team_members!assessments_responsible_id_fkey(display_name)').eq('client_id',client).eq('status','completed').order('completed_at',{ascending:false}).order('id').range(offset,offset+20);
...
async function initialize(){try{
 const access=await getTeamMember();if(!access.member){if(access.reason==='network')throw Error();location.replace('./index.html?reason=expired');return;}updateHeader(access.member);
